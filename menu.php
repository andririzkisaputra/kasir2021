...
<?php
//menu dinamis based on modul hak akses by yuda
$qm = yposSQL('SHOW','ypos_menu','menuID, menu',"aktif='Y' && 1=1",'sort');
while ($menu = $qm->fetch_array()) {
	$qm2 = yposSQL('SHOW','ypos_modul a, ypos_grouplvlmdl b','a.modulID, nama_modul, modul_folder, menuID',"a.modulID=b.modulID && idlevel=$_SESSION[ylevel] && menuID=$menu[menuID] and aktif='1' and 1=1");
	$cekqm2 = $qm2->num_rows;
	if ($cekqm2 > 0) {
...
$qm->free_result();
if ($_SESSION['ylevel'] == 1) { //pantek langsung ke level admin
...
<li><a href="'.$set->folder_modul.'=system&sub=report-param">Report Parameter</a></li>
</ul></li>
</ul>'; }?>