...
<?php
$no =1;
$q = yposSQL('SHOW','ypos_pembelian a, ypos_suplier b','kdPembelian, no_nota, total_pembelian, userID, tgl_input, nama_sup',"a.kdsup=b.kdsup && a.ids=$_SESSION[yids] && 1=1",'tgl_input');
...
<table>
        ...
        <td>:</td>
		<td><input type="text" name="tgl" class="tgl" size="25" <?php if(!empty($kode)) { echo 'value=".'.$ed['tgl'].'"  disabled="disabled"';} else { echo 'value="'.$getDate.'".';} ?>/></td>
		<td></td>
		...
		<td><b>Suplier</b></td>
        <td>:</td>
		<td><div class="styled-select slate semi-square"><select name="sup" <?php if(!empty($kode)) { echo 'disabled="disabled"';}?>>
	...
	while ($rs = $sup->fetch_array()) {
		...
		<td><b>No. Nota</b></td>
        <td>:</td>
		<td><input type="text" name="nota" size="25" value="<?php echo @$_GET['nota'];?>" <?php if(!empty($kode)) { echo 'disabled="disabled"';}?>/></td>
		<td></td>
		<td></td>
		<td></td>
        <td></td>
...
while ($getItem = $Qitem->fetch_array()) {?>
  <tr>
    ...
    <a href="<?php echo $set->folder_modul.'='.$modul?>&act=delete&id=<?php echo $getItem['idDtlPembelian'].'&kdp='.$getItem['kdPembelian'];?>"><img src="images/delete-icon.png" border="0" width="20" height="20" /></a></td>
  </tr>
  <?php $no++; }
  $total = yposSQL('SHOW','ypos_pembeliandtl','DISTINCT SUM(qty_beli) AS t_qty, SUM(harga_beli) AS t_harga',"kdPembelian='$kode' && 1=1")->fetch_array();?>
    ...
    <td align="right"><?php echo idr(@$_GET['ttl']);?></td>
    <td></td>
  </tr>
  <tr>
